...
- 通过环境变量 `FLASK_CONFIG` 在 `development` / `production` / `testing` 配置之间切换，或在 `create_app(test_config={...})` 中直接传入覆盖值。配置定义位于 `config.py`。
- 在生产部署中设置 `FLASK_SECRET`、`ADMIN_USER`、`ADMIN_PASS` 等敏感值。默认配置仅适用于开发环境。
- 表单 CSRF 保护默认启用：安装 `Flask-WTF` 时自动启用官方实现，否则退回到内置 session 方案，并在 `TESTING=True` 时自动跳过验证，方便单元测试。
- 如需在未登录场景下允许临时借用（例如自助终端），设置 `ALLOW_ANONYMOUS_BORROW=1`；默认仅测试环境允许匿名借用。

## 代码结构

- `services/borrowing.py`：封装借用/归还的事务逻辑，提供可复用的 `BorrowService`，后端与 API 共用同一套校验流程。
- `services/auth.py`：集中管理登录、登出、权限装饰器及当前用户缓存，便于未来替换为单点登录等方案。
- `config.py`：集中化配置，便于在不同环境下复用，并可以按需扩展。
